...
async function processLogin(provider, profile, done) {
    try{
        ...
        const [existingUser] = await pool.promise().query('SELECT * FROM User WHERE email = ?',[email]);
        if (existingUser.length > 0) {
            ...
            const [newUser] = await pool.promise().query('SELECT * FROM User WHERE id = ?', [result.insertId])
